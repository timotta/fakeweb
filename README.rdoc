...
=== Registering URI patterns

  FakeWeb.register_uri(:get, /www\.example\.com/, :response => "Hello World!")
  FakeWeb.register_uri(:get, "http://example.com/test3", :string => "Better priority")

You can also register uris by patterns using regular expressions. So, you can
put all pages of an domain, or a subpath, to return the same content. The registered 
patterns has a lower priority than registered absolute uris. So if you register a 
pattern and a uri, if the absolute uri was called, the response registered for it 
will be returned. Other uris will be returned with the response registered with 
the pattern. 
